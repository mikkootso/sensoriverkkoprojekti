    ...
    $.ajax({
      ...
      success : function(data){
        ...
        function drawCards(rajat){
          const cards = [
            {id: 1, title: "Ravinneliuoksen sähkönjohtavuus", min: parseInt(rajat["ec"].min), max: parseInt(rajat["ec"].max), lastValues: {val: data[1][data[1].length-1].ec, stamp: data[1][data[1].length-1].timeStamp, unit: "mS/cm"}}
          ];
        ...
        function drawCharts(){
          ...
          const charts = [
            {cid: "#ec", label: "Ravinneliuoksen sähkönjohtavuus [S/cm]", yData: temp1, color: "rgba(59, 89, 152, 1)", suggestedMin: 10, suggestedMax: 30}
          ];
