    ...
    $.ajax({
      ...
      success : function(data){
        ...
        function drawCards(rajat){
          const cards = [
            {id: 1, title: "Ravinneliuoksen pH", min: parseInt(rajat["ph"].min), max: parseInt(rajat["ph"].max), lastValues: {val: data[1][data[1].length-1].ph, stamp: data[1][data[1].length-1].timeStamp, unit: "pH"}}
          ];
        ...
        function drawCharts(){
          ...
          const charts = [
            {cid: "#ph", label: "Ravinneliuoksen pH [pH]", yData: temp1, color: "rgba(59, 89, 152, 1)", suggestedMin: 4, suggestedMax: 9}
          ];
